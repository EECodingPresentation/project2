...
function phi_output=channel2(phi_input)
    ...
    t=1:length(phi_pulse_I);
    phi_trans=phi_trans_I.*cos(t)+phi_trans_Q.*sin(t);%两路加载载波并相加
    n=normrnd(0,sigma,1,length(phi_trans));
    phi_trans_noisy= phi_trans+n;
    phi_received_I=2*phi_trans_noisy.*cos(t);%I路解调
    ...
    phi_received_Q=2*phi_trans_noisy.*sin(t);%Q路解调
