...
%
%   WARNING: RCOSFIR is not recommended. Use RCOSDESIGN instead.
%
...
%   stream without introducing inter-symbol interference.
%
...
%   sample). The roll-off factor, R, determines the width of the transition
...
%
%   See also RCOSDESIGN, RCOSDEMO, GAUSSDESIGN.

%   Copyright 1996-2016 The MathWorks, Inc.

%routine check
if nargin < 1
    error(message('comm:rcosfir:NotEnoughInputs'))
elseif nargin < 2
...
if ~isscalar(r) || (r < 0) || (r > 1) || ~isreal(r)
    error(message('comm:rcosfir:InvalidR'))
end
...
if ~isscalar(rate) || (rate <= 1) || (ceil(rate) ~= rate)
    error(message('comm:rcosfir:InvalidRate'))
end
